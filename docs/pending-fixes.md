...
- ⚠️ 一处**待你确认的口径差异**：我给的选项预览里把「开」画成了灰白（`--text-secondary`），而你 B-6 原文写的是「开 = **绿色**常亮」。我按**原文**实现（开 = 绿）。若你要的是灰白常亮，改 `inlineStateClass` 末行一处即可，说一声即可。
...
## B-11 PDF 阅读器「边缘被勾画的痕迹」（2026-09-21 立项，P2；2026-09-22 取证：**主推定位已证伪、现象未复现 → 等用户截图**）
...
### ★ 2026-09-22 取证结果（先读这一节；下面「主推定位 A」的推理已被推翻，保留仅供对照）
...
6. 跑这两个探针的环境坑（已解决，供复用）：① 端口用 `KNOWBASE_PROBE_PORT`（本机 9222 被 WorkBuddy 占住）；② `pdf-edge` 要先把 dev 设置的 `currentVaultId` 临时指向放着样书的仓库（`.books` 是**每仓库**的，dev 仓库里没有 PDF），跑完已复原。
...
**修复方向（★ 2026-09-22 起暂停：依据不足，见上面取证第 1/2 条 —— 按 A 落码会被证伪成无效改动）**：
