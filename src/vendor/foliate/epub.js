...
// ── KB PATCH（B 段 · 路 B）：把子资源转成 data: URL ──
// 动机：foliate 默认把一切资源都做成 blob:，逼得宿主必须放开 img/style/font 三处 CSP。
// 改走 data: 后 img-src / font-src 本就允许（现有 CSP 已含 data:），只有内容文档仍需 blob:。
const kbToDataURL = async (data, type) => {
    if (typeof data === 'string')
        return `data:${type || 'text/plain'};charset=utf-8,${encodeURIComponent(data)}`
    let bytes
...
class Loader {
    ...
    async createURL(href, data, type, parent) {
        ...
        // ── KB PATCH：内容文档保持 blob:（paginator 需读 contentDocument，必须同源），
        //    其余子资源（CSS / 图片 / 字体 / SVG）改走 data: ──
    ...
    unref(href) {
        ...
        if (count < 1) {
            ...
            // ── KB PATCH：只回收 blob:（data: 无需 revoke）──
            const kbUrl = this.#cache.get(href)
            if (kbUrl?.startsWith('blob:')) URL.revokeObjectURL(kbUrl)
