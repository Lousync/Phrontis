 ...
 * （**明确不做**）；书签当时定位键（CFI）与存储口径（`TxtBookmark.paraIndex` 是数字）对不上，**未拍板前不出入口**。
 * **2026-09-22 更新**：foliate 系书签已落地（`BookBookmark.cfi`，与 epub/fb2/fbz 同一字段），
 * 但**列表出在右栏「阅读」面板**（`ReadingSidePanel` 的 markItems），本面板仍只出目录 ——
 * 同一份书签不列两处（左栏出 Tab 的时机是"左栏有独立内容"，不是"这功能能用了"）。
 *
