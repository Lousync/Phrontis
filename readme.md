...
- **书架内读书**：六格式原生阅读（PDF / TXT / EPUB / FB2 / FBZ / CBZ）——划选即问 AI、摘录落进阅读面板、书签与全文搜索齐备；书市可在线找书下载入架
...
| 模块 | 说明 |
|------|------|
| ✏️ 编辑器 | **唯一正文写入方**：Monaco 内核 + 文件树（vscode-icons），分栏实时预览、**禅模式**（`Ctrl+K Z`）、**文件操作撤销栈**（`Ctrl+Z` / `Ctrl+Shift+Z` 覆盖移动 / 重命名 / 新建） |
...
| 📖 书架 | 把书拖进仓库即可读：**六种格式原生支持**（PDF / TXT / EPUB / FB2 / FBZ / CBZ，三套阅读引擎）；封面墙与三排列两种摆法、阅读进度自动记住；**划选正文唤起浮条**（AI 讲题 / 翻译 / 摘录 / 复制），摘录高亮落进右栏阅读面板、点击跳回原文；书签（含 PDF 左栏列表）、目录大纲、全文搜索、字号缩放、沉浸阅读与单页 / 竖滚模式 |
| 🛒 书市 | 在线书源检索与下载（内置 Project Gutenberg 等来源），支持自定义书源与凭据管理；下载队列、重复检测、体积分档确认，下到的书直接进书架 |
